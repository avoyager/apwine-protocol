...
library APWineNaming{
    ...
    // i.e. 30D-AAVE-ADAI-2
    function genFYTSymbol(uint8 _index,string memory _ibtSymbol, string memory _platfrom, uint256 _periodDuration) public pure returns (string memory){
    ...
    function genIBTSymbol(string memory _ibtSymbol, string memory _platfrom, uint256 _periodDuration) public pure returns (string memory){
        return concatenate(getPeriodDurationDenominator(_periodDuration),concatenate("-",concatenate(_platfrom,concatenate("-",_ibtSymbol))));
    }
