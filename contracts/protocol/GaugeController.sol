...
contract GaugeController is Initializable, AccessControlUpgradeable{
    ...
    function deployLiquidityGauge(address _future) internal returns(address){
        ...
        ILiquidityGauge newLiquidityGauge = ILiquidityGauge(IProxyFactory(registery.getProxyFactoryAddress()).deployMinimal(registery.getLiquidityGaugeLogicAddress(), payload));
        return address(newLiquidityGauge);
    }   

    function claimAPW(address _future) public { // last fyt claimed -now, 
