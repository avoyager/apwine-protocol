...
abstract contract StreamFutureWallet is FutureWallet {
    ...
    function registerExpiredFuture(uint256 _amount) public override {
        ...
        if (scaledFutureWallets.length > 1) {
            uint256 scaledInput =
                APWineMaths.getScaledInput(_amount, scaledTotal, currentTotal);
        ...
        returns (uint256)
    {
        ...
        uint256 scaledOutput =
            (senderTokenBalance.mul(scaledFutureWallets[_periodIndex]));
        return
            APWineMaths
                .getActualOutput(
                scaledOutput,
    ...
    function _updateYieldBalances(
        ...
        uint256 scaledOutput =
            (_userFYT.mul(scaledFutureWallets[_periodIndex])).div(_totalFYT);
        ...
        scaledFutureWallets[_periodIndex] = scaledFutureWallets[_periodIndex]
            .sub(scaledOutput);
