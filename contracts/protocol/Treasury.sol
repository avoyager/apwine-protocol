...
contract Treasury is Initializable, AccessControlUpgradeable{
    ...
    function sendToken(address _erc20, address _recipient, uint256 _amount) public{
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        IERC20Upgradeable(_erc20).transfer(_recipient,_amount);
    }

    function sendEther(address payable _recipient,uint256 _amount) payable public{
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
