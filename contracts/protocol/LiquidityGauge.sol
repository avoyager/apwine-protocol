...
contract LiquidityGauge is Initializable, AccessControlUpgradeable{
    ...
    function registerNewFutureLiquidity(uint256 _amount) public {
        ...
        totalDepositedSupply[totalDepositedSupply.length-1] = totalDepositedSupply[totalDepositedSupply.length-1].add(_amount);
    ...
    function unregisterFutureLiquidity(uint256 _amount) public { 
        ...
        require(hasRole(FUTURE_ROLE, msg.sender), "Caller is not the corresponding future");
        updateInflatedVolume();
        totalDepositedSupply[totalDepositedSupply.length-1] = totalDepositedSupply[totalDepositedSupply.length-1].sub(_amount);
    ...
    function getLastInflatedAmount() public view returns(uint256){
        return ((gaugeController.getLastEpochInflationRate().mul(supplyStart)).mul(block.timestamp.sub(updatesTimestamp[updatesTimestamp.length-1])).mul(gaugeController.getGaugeWeight()).mul(gaugeController.getGaugeTypeWeight())).div(gaugeController.getEpochLength());
    }

    function getUserRedeemable(address _user) external view returns(uint256){
        return _getRedeemableLiquidityRegistrationOnly(_user).add(_getUserNewRedeemable(_user)).add(userLiquidityRegistered[_user]);
    }
    ...
    function _getUserNewRedeemable(address _user) internal view returns(uint256){
        ...
        for(uint i = userRedeemTimestampIndex[_user]; i< updatesTimestamp.length ;i++){
