...
const {AUSDC_ADDRESS,ADAI_ADDRESS, YUSD_ADDRESS,YDAI_ADDRESS} =  common.mainnet;
...
async function main() {
    ...
    const BETA_STARTING_DATE  = 1612544400; // 5th Feb 2021 6PM CET
    ...
    tx = await ibtFutureFactory.deployFutureWithIBT("AAVE",AUSDC_ADDRESS,7);
    await tx.wait()
    ...
    tx = await ibtFutureFactory.deployFutureWithIBT("YTOKEN",YUSD_ADDRESS,7);
    await tx.wait()
    console.log("Deploy YDAI WEEKLY FUTURE")
    tx = await ibtFutureFactory.deployFutureWithIBT("YTOKEN",YDAI_ADDRESS,7);
    await tx.wait()
